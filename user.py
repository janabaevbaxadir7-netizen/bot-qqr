...
from utils.keyboards import lang_keyboard, main_menu_keyboard
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_adm = is_admin(user.id) or user.id == SUPER_ADMIN_ID
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db_user['is_subscribed']:
        ...
        price = db_user.get('subscription_price', 0) or 0
...
async def cmd_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state:
        ...
        active = get_active_quiz(user.id)
        if active:
...
async def cmd_restart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = get_lang(user.id)
    if user.id in user_quiz_state:
        finish_quiz(user_quiz_state[user.id]['session_id'])
        user_quiz_state.pop(user.id, None)
    await update.message.reply_text(t(lang, 'send_docx'))
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        session_id = create_quiz_session(user.id, quiz_data['name'], len(questions))
        for q in questions:
...
async def send_question(update, context, user_id, lang):
...
async def handle_answer_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    answer_question(state['session_id'], q['num'], answer)
    result_text = "✅ <b>To'g'ri!</b> +1 ball" if is_correct else f"❌ <b>Noto'g'ri!</b>\n✔️ To'g'ri javob: <b>{correct_answer}</b>"
    q_text = (
...
async def send_quiz_result(update, quiz, lang, user_id=None, context=None):
    if not quiz:
        return
...
async def handle_lang_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang_names = {'uz': "✅ Til o'zgartirildi: O'zbek", 'ru': "✅ Язык: Русский", 'en': "✅ Language: English"}
    await query.edit_message_text(lang_names[lang])

async def handle_text_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    user = update.effective_user
    if "Quiz boshlash" in text or "Начать" in text:
        await cmd_quiz(update, context)
    elif "Holat" in text or "Статус" in text:
        await cmd_status(update, context)
    elif "Statistika" in text or "Статистика" in text:
        await cmd_stats(update, context)
    elif "Til" in text or "Язык" in text:
        await cmd_lang(update, context)
    elif "Yordam" in text or "Помощь" in text:
